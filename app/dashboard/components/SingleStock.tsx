...
function SingleStock({ coin }: { coin: Coin }) {
  ...
  return (
    <div className="bg-white shadow-md max-w-md mx-auto flex items-center gap-3 p-4 rounded-xl mb-3">
      {/* Coin Image */}
      <img src={coin.image} alt={coin.name} className="w-8 h-8 rounded-full" />

      <div className="flex-1">
        <h2 className="font-semibold">{coin.name || "demo coin"}</h2>
      ...
      <div className="text-right">
        <p className="font-bold ">
          {getRsFromUsd(coin.current_price) || 999}/-
        </p>
        <p
